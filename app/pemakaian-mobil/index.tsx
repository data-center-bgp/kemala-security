...
export default function PemakaianMobilScreen() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (dateFilter !== "all") {
      ...
      if (dateFilter === "today") {
        result = result.filter((item) => item.tanggal_pakai === todayStr);
      } else if (dateFilter === "week") {
        const weekAgo = new Date(now);
        weekAgo.setDate(weekAgo.getDate() - 7);
        const weekStr = weekAgo.toISOString().split("T")[0];
        result = result.filter((item) => item.tanggal_pakai >= weekStr);
      } else if (dateFilter === "range") {
        const fromStr = formatDateStr(dateFrom);
        const toStr = formatDateStr(dateTo);
        result = result.filter(
          (item) =>
            item.tanggal_pakai >= fromStr && item.tanggal_pakai <= toStr,
        );
      }
    }
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((item) => {
        ...
        const mobilStr = mobil
          ? `${mobil.brand} ${mobil.nama} ${mobil.nomor_plat}`.toLowerCase()
          : "";
        return (
          item.nama_peminjam?.toLowerCase().includes(q) ||
          item.keperluan?.toLowerCase().includes(q) ||
          item.tanggal_pakai?.toLowerCase().includes(q) ||
          mobilStr.includes(q)
        );
      });
    }
      ...
      <View style={styles.card}>
        <View style={styles.cardTop}>
          <View style={styles.cardIconWrap}>
            <MaterialCommunityIcons name="car" size={20} color="#0a7ea4" />
          </View>
          <View style={styles.cardTitleArea}>
            ...
            <View style={styles.dateBadge}>
              ...
              <Text style={styles.dateText}>
                {item.tanggal_pakai} • {item.waktu_pakai}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.cardActions}>
          <TouchableOpacity
            ...
            onPress={() =>
              router.push(`/pemakaian-mobil/edit?id=${item.id}` as any)
            }
            activeOpacity={0.7}
          >
            <MaterialCommunityIcons
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Pemakaian Mobil</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.searchBar}>
        ...
        <TextInput
          ...
          placeholder="Cari nama, mobil, keperluan..."
            ...
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.emptyText}>
                  Belum ada data pemakaian mobil
                </Text>
              </>
            )}
          </View>
      ...
      <View style={styles.footer}>
        ...
        >
          ...
          <Text style={styles.addButtonText}>Tambah Pemakaian Mobil</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
