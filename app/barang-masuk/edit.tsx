...
import { FotoBarangMasuk } from "@/types/database";
...
export default function EditBarangMasuk() {
  ...
  const [existingPhotos, setExistingPhotos] = useState<FotoBarangMasuk[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setTime(t);
      setPengirim(row.pengirim ?? "");
      setPenerima(row.penerima ?? "");
      setBarang(row.barang ?? "");
      setKeterangan(row.keterangan ?? "");

      // Load existing photos with fresh signed URLs
      if (row.foto_barang_masuk?.length) {
        const photos = await Promise.all(
          row.foto_barang_masuk.map(async (foto: FotoBarangMasuk) => {
  ...
  const removeExistingPhoto = (foto: FotoBarangMasuk) => {
  ...
  const deleteRemovedPhotos = async () => {
    for (const photoId of removedPhotoIds) {
      const { data: foto } = await supabase
        .from("foto_barang_masuk")
        .select("storage_path")
        .eq("id", photoId)
        .single();

      if (foto?.storage_path) {
        await supabase.storage.from("barang_masuk").remove([foto.storage_path]);
      }
      await supabase.from("foto_barang_masuk").delete().eq("id", photoId);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Barang Masuk</Text>
        <View style={{ width: 40 }} />
      </View>
